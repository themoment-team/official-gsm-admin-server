...
public class UserController {
    ...
    @Operation(
            summary = "이름 등록 요청",
            description = "oauth로그인 후 이름을 등록하는 api",
            tags = {"User Controller"}
    )
    public ResponseEntity<Void> nameSet(
    ...
    @Operation(
            summary = "승인대기 회원정보 조회 요청",
            description = "승인대기중인 회원정보를 조회하는 api",
            tags = {"User Controller"}
    )
    ...
    @Operation(
            summary = "회원을 승인하는 요청",
            description = "승인대기중인 회원을 승인하는 api",
            tags = {"User Controller"}
    )
    public ResponseEntity<Void> approved(
            @Parameter(name = "userSeq", description = "user의 seq값", in = ParameterIn.PATH)
    ...
    @Operation(
            summary = "회원 승인을 거절하는 요청",
            description = "승인대기중인 회원을 승인 거절하는 api",
            tags = {"User Controller"}
    )
    public ResponseEntity<Void> refuseApproved(
            @Parameter(name = "userSeq", description = "user의 seq값", in = ParameterIn.PATH)
            @PathVariable Long userSeq
    ){
